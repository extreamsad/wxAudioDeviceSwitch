...
void MyTaskBarIcon::OnShowPopupMenu(wxUpdateUIEvent & event)
{		
	//event.Check(true);
}
...
{		
	...
	for (auto &d : MainForm::GetAudioPlaybackDevices())
	{
		...
		this->Connect(devs_menu->GetId(), wxEVT_UPDATE_UI, wxUpdateUIEventHandler(MyTaskBarIcon::OnShowPopupMenu));
	}
